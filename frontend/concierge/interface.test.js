...
function fixture(fetch) {
  ...
  const makeNode=()=>({
    listeners:{},classList:{values:new Set(),add(x){this.values.add(x);},remove(x){this.values.delete(x);},contains(x){return this.values.has(x);},toggle(){}},
  ...
  const context=vm.createContext({document,fetch,validPage,followupDraft,isUserCaption,URL,location:{origin:'https://example.test'},innerWidth:1280,
    window:{addEventListener(){}},parent:{postMessage(message){messages.push(message);}},crypto:{randomUUID:()=> 'test-id'},
  ...
  return {node,context,body,messages,connections:()=>connections};
}
